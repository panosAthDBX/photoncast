...
impl ExtensionListView {
    ...
    fn render_search_input(
        ...
        if has_query {
            let before_cursor =
                &self.search_query[..self.cursor_position.min(self.search_query.len())];
            let after_cursor =
                &self.search_query[self.cursor_position.min(self.search_query.len())..];
    ...
    fn render_list_content(
        ...
        let preview_item = if has_preview {
            self.selected_item()
                .and_then(|item| item.item.preview.clone().into_option())
        } else {
            None
        };
        ...
        div()
            ...
            .when_some(if has_preview { preview_item } else { None }, |el, item| {
                el.child(
                    div()
                        .w(PREVIEW_WIDTH)
                        .border_l_1()
        ...
        match icon {
            ...
            IconSource::AppIcon { icon_path, .. } => {
                if let ROption::RSome(path) = icon_path {
                    div().w(ICON_SIZE_LG).h(ICON_SIZE_LG).child(
    ...
    fn render_accessory(&self, accessory: &Accessory, colors: &ExtensionViewColors) -> gpui::Div {
        match accessory {
            Accessory::Text(text) => div()
                .text_xs()
                .text_color(colors.text_muted)
                .child(text.to_string()),
            Accessory::Tag { text, color } => div()
                .px(px(6.0))
                .py(px(2.0))
                .rounded(px(4.0))
                .text_xs()
            ...
            Accessory::Date(duration) => {
                ...
                div().text_xs().text_color(colors.text_muted).child(text)
            },
            Accessory::Icon(icon) => self.render_icon(icon, colors),
        }
    }
    ...
    fn render_empty_state(
        ...
        let icon_content: Option<String> =
            empty_state
                .icon
                .clone()
                .into_option()
    ...
    fn render_loading(&self, colors: &ExtensionViewColors) -> impl IntoElement {
        div()
            ...
            .items_center()
            .justify_center()
            .child(div().text_color(colors.text_muted).child("Loading..."))
    }
    ...
    fn render_action_bar(&self, colors: &ExtensionViewColors) -> impl IntoElement {
        ...
        let has_actions = self
            .selected_item()
            .is_some_and(|item| !item.item.actions.is_empty());
