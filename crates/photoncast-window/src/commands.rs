...
impl WindowCommand {
    ...
    /// Activates a specific app by bundle ID.
    #[cfg(target_os = "macos")]
    pub fn activate_app(&self, bundle_id: &str) -> crate::Result<()> {
        let manager = self.manager.read();
        manager.activate_app(bundle_id)
    }
